...
split = int(len(jfiles)*0.8)
train_j = jfiles[:split]; val_j = jfiles[split:]

for split_name, jlist in [('train',train_j), ('val',val_j)]:
    cnt=0
    for jf in jlist:
        ...
        shutil.copy(imgp, dst/'images'/split_name/imgp.name)
        (dst/'labels'/split_name/(jf.stem+'.txt')).write_text('\n'.join(lines))
        cnt+=1
